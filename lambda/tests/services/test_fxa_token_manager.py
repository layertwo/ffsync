...
class TestCreateSessionToken:
    ...
    @pytest.fixture
    def manager(self, dynamodb_table: "Table") -> FxATokenManager:
...
class TestVerifySessionHawk:
    ...
    def test_returns_none_for_missing_header(self, manager: FxATokenManager) -> None:
    ...
    def test_reraises_non_conditional_nonce_error(
        ...
        with pytest.raises(ClientError) as exc_info:
            manager.verify_session_hawk(auth_header, "GET", "/v1/session/status", "localhost", 443)
        assert exc_info.value.response["Error"]["Code"] == "InternalServerError"
...
class TestVerifyKeyfetchHawkEdgeCases:
    ...
    def test_returns_none_when_receiver_skips_credentials_map(
        self, manager: FxATokenManager
    ) -> None:
...
class TestConstants:
    ...
    def test_session_token_info(self) -> None:
        assert SESSION_TOKEN_INFO == "identity.mozilla.com/picl/v1/sessionToken"

    def test_key_fetch_token_info(self) -> None:
