...
@pytest.fixture
def route(mock_account_manager: MagicMock) -> ScopedKeyDataRoute:
...
class TestScopedKeyData:
    def test_success_returns_key_data(
        self, route: ScopedKeyDataRoute, mock_account_manager: MagicMock
    ) -> None:
    ...
    def test_missing_body_returns_400(self, route: ScopedKeyDataRoute) -> None:
        event = _make_event(body=None)
        response = route.handle(event)
        assert response.status_code == 400

    def test_invalid_json_body_returns_400(self, route: ScopedKeyDataRoute) -> None:
    ...
    def test_missing_scope_returns_400(self, route: ScopedKeyDataRoute) -> None:
        event = _make_event(
...
class TestScopedKeyDataBind:
    def test_bind_registers_post_route(self, route: ScopedKeyDataRoute) -> None:
        mock_api = MagicMock()
