...
- `delete_all_storage()` supports complete user data deletion (Requirement 4.3)
- `get_collection_counts()` and `get_collection_usage()` provide efficient metadata queries without loading all BSOs (Requirements 6.2, 6.3)
...
class BasicStorageObject:
    ...
    sortindex: Optional[int]   # Sort order (-2147483648 to 2147483647)
...
class CollectionData:
    name: str                  # Collection name (max 32 chars, [a-z0-9_])
...
class BatchResult:
    ...
    failed: Dict[str, List[str]]  # ID -> list of error messages
...
| expiry | Number | DynamoDB TTL attribute (epoch seconds) |
...
MAX_TOTAL_RECORDS = None                  # Unlimited
...
*For any* valid BSO with id, payload, sortindex, and ttl, storing it via PUT and then retrieving it via GET SHALL return an equivalent BSO with matching id, payload, sortindex, and ttl fields.
...
*For any* collection containing BSOs, a GET request to `/storage/{collection}` SHALL return all BSO IDs in the collection, and with `full=true` SHALL return complete BSO objects.

**Validates: Requirements 2.1, 2.2**

### Property 5: ID Filtering Correctness

*For any* collection and subset of BSO IDs, a GET request with `ids={id1},{id2},...` SHALL return exactly the BSOs with those IDs and no others.
...
- A request with `offset=m` SHALL skip the first m BSOs
- When more results exist, `X-Weave-Next-Offset` header SHALL be present
...
- `failed` object mapping failed IDs to error arrays

**Validates: Requirements 3.1, 3.2, 3.3**

### Property 10: Selective Deletion Correctness

*For any* collection and subset of BSO IDs, a DELETE request with `ids={id1},{id2},...` SHALL delete exactly those BSOs while preserving others in the collection.

**Validates: Requirements 4.1**

### Property 11: Collection Deletion Completeness

*For any* collection, a DELETE request without IDs SHALL delete all BSOs in that collection, and a DELETE to `/storage` SHALL delete all collections for the user.

**Validates: Requirements 4.2, 4.3, 4.4**

### Property 12: Optimistic Concurrency Control

*For any* write request with `X-If-Unmodified-Since` header where the collection has been modified after that timestamp, the server SHALL return a 412 Precondition Failed status code.
...
### Property 15: BSO ID Validation

*For any* BSO ID containing characters outside printable ASCII (0x20-0x7E), the server SHALL return a 400 status code.
...
*For any* error response, the body SHALL be valid JSON containing error information.

**Validates: Requirements 12.1, 12.2**

### Property 18: Collection Name Validation

*For any* collection name:
- Names exceeding 32 characters SHALL return 404
- Names containing only lowercase letters (a-z), digits (0-9), and underscores SHALL be accepted
- Names containing other characters SHALL return 404

**Validates: Requirements 14.1, 14.2, 14.3**

### Property 19: BSO Validation Completeness

*For any* BSO submission:
- Payloads exceeding `max_record_payload_bytes` (256 KB) SHALL return 400
- IDs exceeding 64 characters SHALL return 400
- Sortindex values outside [-2147483648, 2147483647] or non-integer values SHALL return 400
- TTL values that are negative or non-integer SHALL return 400
...
### Property 21: Authentication Enforcement (Lambda Authorizer)
...
**Success Responses (Requirement 15.1-15.4):**

| Status | Operation | Response Body |
|--------|-----------|---------------|
| 200 | GET BSO | BSO object with id, payload, modified, sortindex, ttl |
...
| 200 | DELETE | Deletion timestamp (float) |
| 200 | GET /info/* | JSON object with requested metadata |

**Error Responses (Requirement 15.5-15.9):**

| Status | Condition | Exception |
|--------|-----------|-----------|
| 400 | Invalid request/validation error | `ValidationException` |
| 400 | Batch exceeds max_post_records | `ValidationException` |
| 401 | Authentication failure | `AuthenticationException` |
| 404 | BSO or collection not found | `StorageObjectNotFoundException`, `CollectionNotFoundException` |
| 404 | Invalid collection name | `CollectionNotFoundException` |
| 409 | Conflict | `ConflictException` |
| 412 | Precondition failed | `PreconditionFailedException` |
| 413 | Request body too large | `RequestTooLargeException` |
| 413 | Batch payload exceeds max_post_bytes | `RequestTooLargeException` |
| 500 | Internal server error | Generic `Exception` |
| 507 | Quota exceeded | `QuotaExceededException` |

### Error Response Format

All error responses return valid JSON with consistent structure:
...
|----------|--------|---------------|
| BSO not found | 404 | `{"error": "NotFound", "message": "BSO not found"}` |
| Collection not found | 404 | `{"error": "NotFound", "message": "Collection not found"}` |
| Validation error | 400 | `{"error": "ValidationError", "message": "BSO ID exceeds 64 characters"}` |
| Batch record limit exceeded | 400 | `{"error": "ValidationError", "message": "Batch exceeds max_post_records limit"}` |
| Request too large | 413 | `{"error": "RequestTooLarge", "message": "Request body exceeds max_request_bytes"}` |
| Batch payload too large | 413 | `{"error": "RequestTooLarge", "message": "Batch payload exceeds max_post_bytes"}` |
| Precondition failed | 412 | `{"error": "PreconditionFailed", "message": "Collection modified since X-If-Unmodified-Since"}` |
| Server error | 500 | `{"error": "InternalError", "message": "An internal error occurred"}` |

**Validates: Requirements 12.1, 12.2, 12.3, 12.4, 12.5**

### Batch Limit Validation

The server enforces three distinct limits on batch operations with specific status codes:

| Limit | Value | Violation Status | Requirement |
|-------|-------|------------------|-------------|
| `max_post_records` | 100 BSOs | 400 Bad Request | 3.4 |
| `max_post_bytes` | 2 MB | 413 Request Too Large | 3.5 |
| `max_request_bytes` | 2 MB | 413 Request Too Large | 3.6 |

**Design Rationale:** The distinction between 400 and 413 follows HTTP semantics:
- 400 indicates a logical validation error (too many records)
- 413 indicates the request entity is too large (payload size)

### Validation Rules

1. **BSO ID**: Max 64 characters, printable ASCII only (0x20-0x7E)
2. **Collection Name**: Max 32 characters, lowercase alphanumeric and underscore only
3. **Payload Size**: Max 256 KB per BSO
4. **Batch Size**: Max 100 BSOs per POST, max 2 MB total
5. **Sortindex**: Integer in range [-2147483648, 2147483647]
6. **TTL**: Non-negative integer
...
# Valid collection name: 1-32 lowercase alphanumeric + underscore
...
valid_bso_strategy = st.builds(
    ...
    sortindex=st.one_of(st.none(), st.integers(min_value=-2147483648, max_value=2147483647)),
...
│   ├── test_bso_properties.py     # Properties 1-3, 15-16, 19
│   ├── test_collection_properties.py  # Properties 4-8, 10-11, 13, 18
│   ├── test_batch_properties.py   # Property 9
│   ├── test_concurrency_properties.py  # Property 12
│   ├── test_response_properties.py    # Properties 14, 17, 20
│   └── test_auth_properties.py    # Property 21 (Lambda Authorizer)
